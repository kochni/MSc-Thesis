...
class StateSpaceModel:
    ...
    * `logeta(self, t, x, data)`: the auxiliary weight function at time t
    ...
    def simulate_given_x(self, x):
        lag_x = [None] + x[:-1]
        return [
            self.PY(t, x).rvs(size=1) for t, (xp, x) in enumerate(zip(lag_x, x))
        ]
...
class Bootstrap(particles.core.FeynmanKac):
...
class ChopinStochVol(StateSpaceModel):
...
class StochVolLeverage(ChopinStochVol):
